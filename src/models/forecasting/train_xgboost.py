"""Train a real XGBoost model for M5 store-level forecasting."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train XGBoost for M5")
    ...
    parser.add_argument("--model-path", default="models/xgb_model_CA_1.bin", help="Output model artifact path")
    parser.add_argument("--metrics-path", default="models/xgb_model_CA_1.metrics.json", help="Output metrics path")
...
def main() -> None:
    ...
    model, metrics = train_xgboost_model(train_frame, validation_frame, feature_columns)
    ...
    pred_long["sales"] = model.predict(validation_frame[feature_columns])["median"]
