...
from src.simulation.entities import SimulationDailyMetrics
    ...
    def get_metrics(self) -> Dict[str, float]:
        if not self.daily_metrics:
            return {
                ...
                "service_level": 1.0, "waste_rate": 0.0,
                "total_waste_units": 0.0, "total_lost_sales": 0.0,
                "on_time_delivery_rate": 1.0, "days_simulated": 0.0,
            }

        m = self.daily_metrics
        total_days      = len(m)
        total_revenue   = sum(x.total_revenue   for x in m)
        total_profit    = sum(x.daily_profit     for x in m)
        total_demand    = sum(x.total_demand     for x in m)
        total_sold      = sum(x.total_sold       for x in m)
        total_lost      = sum(x.total_lost_sales for x in m)
        total_waste     = sum(x.waste_units      for x in m)
        stockout_days   = sum(1 for x in m if x.stockout_occurred)

        avg_fill_rate   = float(np.mean([x.fill_rate for x in m]))
        stockout_rate   = stockout_days / total_days
        service_level   = (total_sold / total_demand) if total_demand > 0 else 1.0
        waste_rate      = (total_waste / total_demand) if total_demand > 0 else 0.0
        on_time         = (
            self._on_time_deliveries / self._total_deliveries
            if self._total_deliveries > 0 else 1.0
        )
        ...
        return {
            ...
            "days_simulated":        float(total_days),
        }
